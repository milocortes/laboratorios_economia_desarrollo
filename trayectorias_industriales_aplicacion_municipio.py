...
def _(mo):
    ...
    Usaremos los resultados de las regresiones para estimar probabilidades de aparición de industrias para un municipio en específico.

    Reestimaremos la regresion de aparición de industrias con la especificación completa con ```scikit-learn``` para poder inferir las probabilidades.
    """
    )
    return
...
@app.cell
def _(LogisticRegression, df_regresiones_adicionales):
    ### Obtenemos los arreglos de características y target
    caracteristicas = ["density", "input_presente", "output_presente", "similaridad_input_presente", "similaridad_output_presente"]

    y = df_regresiones_adicionales["apariciones"].to_numpy()
    X_reg = df_regresiones_adicionales[caracteristicas]

    ### Entrenamos la regresión logística
    clf = LogisticRegression(random_state=0).fit(X_reg, y)

    return caracteristicas, clf
...
def _(mo):
    mo.md(
        r"""
    Estimaremos las probabilidades de aparición de industrias para el municipio de Aguascalientes con clave 01001 usando la regresión logística estimada.

    Haremos un subset de los datos para quedarnos sólo con Aguascalientes así como sus industrias con RCA < 1.0 en  $t_1$.
    """
    )
    return
...
@app.cell
def _(caracteristicas, clf, df_regresiones_adicionales, pl):
    ### Obtenemos características de Aguascalientes para las industrias no especializadas
    df_ags_caracteristicas = df_regresiones_adicionales.filter(
        (pl.col("cve_geo_mun")=="01001") &
        (pl.col("rca_t1") < 1.0)
    ).select(["subs_id"] + caracteristicas)

    np_ags_caracteristicas = df_ags_caracteristicas.select(caracteristicas).to_numpy()

    ### Estimamos las probabilidades de aparición
    probabilidades_aparicion = clf.predict_proba(np_ags_caracteristicas)[:,1]

    ### Agregamos los resultados al dataframe
    df_ags_caracteristicas = df_ags_caracteristicas.with_columns(
        prob_aparicion = probabilidades_aparicion
    )

    ### Ordenamos de acuerdo a la probabilidad de aparición
    df_ags_caracteristicas = df_ags_caracteristicas.sort("prob_aparicion", descending=True)
    df_ags_caracteristicas
    return (df_ags_caracteristicas,)
...
def _(mo):
    mo.md(
        r"""
    Para Aguascalientes, las tres industrias con mayor probabilidad de aparición fueron:

    * 311 :  Industria alimentaria
    * 312 :  Industria de las bebidas y del tabaco
    * 321 : Industria de la madera.

    Visualicemos una gráfica de dispersión tomando densidad y probabilidad de aparición para identificar donde se encuentran estas industrias. Como era de esperarse, las tres industrias son las mejor rankeadas al comparar densidad y probabilidad de aparición.
    """
    )
    return
...
@app.cell
def _(df_ags_caracteristicas):
    import altair as alt

    # Create the scatter plot points
    points = alt.Chart(df_ags_caracteristicas).mark_point().encode(
        alt.X('density:Q', 
              scale=alt.Scale(zero=False), 
              title = "Densidad"),
        alt.Y('prob_aparicion:Q', 
             title= "Probabilidad de Aparición")
    ).properties(
        title='Densidad vs Probabilidad de Aparición'
    )
    ...
    text = points.mark_text(
        align='left',
        baseline='middle',
        dx=7  # Offset the text slightly from the point
    ).encode(
        text='subs_id'
    )

    # Combine the points and the text labels
    chart = points + text
    chart
    return
